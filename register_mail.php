...
if (isset($_POST['submit'])) {
   //メールアドレス空欄の場合
    ...
    if(!preg_match("/^([a-zA-Z0-9])+([a-zA-Z0-9\._-])*@([a-zA-Z0-9_-])+([a-zA-Z0-9\._-]+)*.dendai.ac.jp+$/", $mail)){
   ...
   //エラーがない場合、pre_accountテーブルにインサート
    if (count($errors) === 0){
        ...
        $url = "http://retwitter.rd.dendai.ac.jp/test//register.php?urltoken=".$id;
       //ここでデータベースに登録する
        try{
           //例外処理を投げる（スロー）ようにする
