...
foreach ($stmt -> fetchAll(PDO::FETCH_ASSOC) as $message){
    ...
    <div class =ret_post_main>
        ...
        <?php if($message['retweet'] != NULL){
            ...
            select_retweet($message['retweet']);
        } else{ ?>
            <!-- 投稿日時を表示 -->
            <span class = 'dt'><?php echo $message['dt']; ?></span>
            ...
            <div class = 'post' data-postid = '<?php echo $message['post_num']; ?>'>
                <div class= "btn-good <?php if(isGood($_SESSION['userid'], $message['post_num'])) echo 'active'; ?>" >
                    ...
                    <i class='fa fa-heart
                        <?php if(isGood($_SESSION['userid'],$message['post_num'])){ //いいね押したらハートが塗りつぶされ>る
                            echo ' active fas';
        ...
        <span class ='fa fa-comment'></span> ,<?php echo $message['commnet']; ?>
    </div>
    <div class = 'postr' data-postid = '<?php echo $message['post_num']; ?>'>
        <div class= "btn-retweet <?php if(isRetweet($_SESSION['userid'], $message['post_num'])) echo 'act'; ?>" >
            ...
            '></i>
            <span><?php echo $message['r_num']; ?></span>
        </div>
    </div>
<?php } ?>
</div>
...
function select_retweet($p_id){
    ...
    $st_img = select_img($id);
    $image = $st_img->fetch();
    echo "<span class = 'dt'>",$message['dt']."</font></span>";
    if($message['user_id'] == $_SESSION['userid']){
        echo "<form method='GET' action = 'mypage.php'>";
        echo "<a href =\"mypage.php\" id='clear_sample'>"; // リンクを付与
        if($image['dot'] == 'jpg'){
            echo "<span class = 'name' id = 'my_post'><img src = 'data:image/jpg;base64, ". base64_encode($image['image']). "' width = '30' height = '30'>",$message['name']."<font class = 'log-id'>  ",$message['log_id']."</font></span> ";
        } else if($image['dot'] == 'png'){
            echo "<span class = 'name' id = 'my_post'><img src = 'data:image/png;base64, ". base64_encode($image['image']). "' width = '30' height = '30'>",$message['name']."<font class = 'log-id'>  ",$message['log_id']."</font></span> ";
        } else if($image['dot'] == 'gif'){
            echo "<span class = 'name' id = 'my_post'><img src = 'data:image/gif;base64, ". base64_encode($image['image']). "' width = '30' height = '30'>",$message['name']."<font class = 'log-id'>  ",$message['log_id']."</font></span> ";
        } else{
            echo "<span class = 'name' id = 'my_post'><img src='images/user.png' width='30' height='30'>",$message['name']."<font class = 'log-id'>  ",$message['log_id']."</font></span> ";
        }
        echo "</form>";
