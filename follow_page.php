...
<html>
    ...
    <body>
        ...
        $NamePage = $result['name']."のフォロー一覧";
        require('header.php');
            
        echo "<main>";
            echo "<a href='#top' id='page_top'><span class ='fa fa-angle-up'></span></a>";

            $stmt = select_follow($follow_id);
            ...
            // DBから投稿内容を取得する関数
            function select_follow($id){
                $dbh = connectDB();
                $sql = 'SELECT account.user_id, account.log_id, account.name, account.image, account.dot FROM account, follow WHERE follow.follower = :id AND follow.follow = account.user_id';
