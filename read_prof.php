...
<?php
    $stmt = select();
    $stmts = select_prof();
    $st_follow = select_follow();
    $prof = $stmts->fetch(PDO::FETCH_ASSOC);
?>
<BR><div class ='prof_border'>
<!-- マイページのプロフィールアイコンの記述 -->
...
$ff = $st_follow->fetch(PDO::FETCH_ASSOC);
$follow = $ff["id"];
...
foreach ($stmt -> fetchAll(PDO::FETCH_ASSOC) as $message){
    ...
    <div class =ret_post_main>
        ...
        <?php if($message['ret_num'] != NULL){
            ...
            <input type = 'hidden' name = 'retweet' value = "<?php echo $message['retweet']; ?>">
            <input type = 'hidden' name = 'del_ret' value = "1">
            <button type = 'submit' name = 'submit' id = 'submit'><span class ='fa fa-trash'></span> 削除</button>
            </form>
            <?php }
...
function select(){
    ...
    $sql = "SELECT *
	        FROM account, post
	        WHERE account.user_id=post.user_id AND :p_id = post.user_id
	        ORDER BY date DESC
            LIMIT 100";
...
function select_retweet($p_id){
    ...
    if($message['user_id'] == $_SESSION['userid']){
        ...
        echo "<button type = 'submit' name = 'submit' id = 'submit'><span class ='fa fa-trash'></span> 削除</button>";
        echo "</form>";
    } else{
...
//フォローしてるかを取得
function select_follow(){
    $dbh = connectDB();
    $user_id = $_SESSION["userid"];
    $page_id = $_SESSION["page"];
    $fol = "SELECT id
	        FROM follow
	        WHERE follower = ". $user_id ." AND ". $page_id ." = follow";
    $st_follow = $dbh -> prepare($fol);
    $st_follow -> execute();
    return $st_follow;
}
...
/* アカウントテーブルからプロフィールを取得  */
function select_prof(){
    $dbh = connectDB();
    $user_id = $_SESSION["page"];
